...
internal class DiDecoder{
    ...
    internal static func getJson2VerificationResultDecoder() -> JSONDecoder {
        ...
        dec.dateDecodingStrategy = .custom { decoder in
            ...
            // сначала пробуем число (секунды или миллисекунды)
            if let n = try? c.decode(Double.self) {
                return n > 10_000_000_000
                    ? Date(timeIntervalSince1970: n / 1000)
                    : Date(timeIntervalSince1970: n)
            ...
            DiLogger.shared.e("Unsupported date format in VerificationResult decoder: \(s)", tag: "VerificationResult")
            throw DecodingError.dataCorruptedError(in: c, debugDescription: "Unsupported date: \(s)")
        }
    ...
    internal static func getJson2TokenDecoder() -> JSONDecoder {
        let dec = JSONDecoder()
        
        dec.dateDecodingStrategy = .custom { decoder in
            let c = try decoder.singleValueContainer()
            
            // сначала пробуем как число (секунды или миллисекунды)
            if let n = try? c.decode(Double.self) {
                return n > 10_000_000_000
                    ? Date(timeIntervalSince1970: n / 1000)
                    : Date(timeIntervalSince1970: n)
            ...
            DiLogger.shared.e("Unsupported date format in TokenDecoder: \(s)", tag: "TokenResult")
            throw DecodingError.dataCorruptedError(in: c, debugDescription: "Unsupported date: \(s)")
        }
